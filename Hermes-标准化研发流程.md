...
3. **确认系统管理初始化数据（一次性问完）：**
   - **用户：** 系统需要哪些用户？默认系统管理员 admin
   - **角色：** 需要哪些角色？每个角色有哪些菜单/功能权限？
   - **部门：** 需要哪些部门？
   - **机构：** 需要哪些机构（5级树形）？
   - **菜单/功能：** 业务模块的一级/二级菜单确认后，补充到菜单和功能表
   - 确认的数据写入 `sql/init.sql` 中的 INSERT 语句
   - **后续开发新功能时：** 菜单和功能的种子数据要同步更新到 `sql/init.sql`
   - **⛔ 门禁：`grep -c "INSERT INTO" sql/init.sql` 确认至少包含用户/角色/部门/机构/菜单/功能的 INSERT 语句，INSERT 数量 ≥ 5 条，否则不能进入下一步（批量生成原型）**
...
4. **批量生成原型（有前端时）：**
   ...
   - 启动 dev server
   - **同步侧边栏菜单：** 批量生成业务页面后，同步更新 `Layout.vue` 中的 `topMenus` 数组（新增对应菜单项），删除示例页菜单（`MENU_EXAMPLE`）
   - **提供访问地址：** 启动 dev server 后，明确告知用户访问 URL（`http://localhost:{端口}`）
...
**菜单/功能同步：** 如果当前 Task 新增了页面或功能，同步更新 `sql/init.sql` 中的 tb_menu 和 tb_function INSERT，确保种子数据与代码一致
...
- 有头模式（`headless: false`），正常速度，`fill()` 快速填表
- 覆盖全部用户测试案例，逐条验证
...
- 清理测试截图（可选）
- **恢复环境到可验收状态：数据库只保留 `sql/init.sql` 中的种子数据（用户/角色/部门/机构/菜单/功能等系统管理数据），不包含业务数据（合作方/订单等由用户验收时自行新增）**

...
> **与阶段4 E2E 测试的职责划分：**
> - 阶段4 E2E：**自动化测试**覆盖常规路径和6类盲区（入口/路由/UI/类名/组件库/弹窗），运行快，可重复
> - 阶段4.5 功能验收：**人工走查**覆盖自动化测试覆盖不到的"真实用户体验"——边界场景、权限组合、无数据显示、异常提示、操作手感
> - 两者的关系：E2E 保证"系统不崩"，功能验收保证"系统好用"。功能验收发现的共性盲区应回灌到 E2E 盲区清单。
...
4. 发现问题 → 记录到临时清单
   - 小 bug（前端显示问题、参数校验遗漏等）：**直接修复**，在 feat 分支提交 `git commit -m "fix: {功能名} {问题描述}"`，**不必重回阶段4**
   - 大 bug（功能缺失、架构设计缺陷、核心逻辑错误）：**停止功能验收**，退回阶段2（架构）或阶段1（需求）修正后，从阶段2/阶段1重新执行
   - 不论大小，修复后必须重新跑集成测试通过（阶段4 步骤2），再进入功能验收重新验证本功能路径
...
规范审查：需求覆盖？Spec场景通过？超范围功能？边界情况？
  - 对照 docs/requirements/{功能名}.md 中的边界条件清单，逐条确认已在测试或验收中覆盖

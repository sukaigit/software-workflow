...
| 调整类型 | 通用实例 | 影响范围 |
|:---------|:-----------|:-----------|
| **模块名变更** | 功能模块改名 | 同步更新 spec + prototype + 权限/路由 |
| **功能拆分** | 一个模块拆为多个子页面 | 新增 route + render + permission |
| **字段增删** | 实体新增/删除字段 | 同步更新 spec + prototype + 数据模型 |
| **业务逻辑** | 状态流转/条件渲染/角色过滤 | 原型条件渲染 + spec Scenario 更新 |
| **数据驱动** | 下拉/列表从 API 动态加载 | 禁止硬编码 option/list 数据 |
| **权限细化** | 审批链/数据权限按角色区分 | 影响原型数据过滤逻辑 |
**门禁通过后提交：**
```bash

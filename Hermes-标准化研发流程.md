...
9. **同步更新侧边栏菜单：** 生成新业务页面后，同步更新 `Layout.vue` 中的 `topMenus` 数组（新增对应菜单项），删除示例页菜单（`MENU_EXAMPLE`）
10. **提供访问地址：** 启动 dev server 后，明确告知用户访问 URL（`http://localhost:{端口}`），并要求用户翻看所有页面列出问题
